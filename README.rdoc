...
This plugin was originally based on ActsAsTaggableOnSteroids by Jonathan Viney.
...
Enter ActsAsTaggableOn. Rather than tying functionality to a specific keyword
...
This variant of ActsAsTaggableOn does not support Rails 2.3.x.  This branch has
had compatibility removed to allow more Rails-esque code.


=== Rails 3.0

ActsAsTaggableOn is now useable in Rails 3.0, thanks to the excellent work of Szymon Nowak
...
2. rake db:migrate
3. Create two model classes, like so:
...
These classes form your extension-point for changing the behavior of the code.



== Testing

ActsAsTaggableOn uses RSpec for its test coverage. Inside the plugin
